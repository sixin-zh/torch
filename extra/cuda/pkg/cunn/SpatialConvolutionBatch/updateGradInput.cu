...
// default: B_Y = B_X = dModuleCache = 16
//          numImages % 16 == 0
...
template < int B_Y, int B_X, int imgsPerThread, int colorsPerThread, int dModuleCache, int nocolorCheck >
...
(
  ...
  for (int cdm = 0; cdm < modLoads; cdm += dModuleCache) {
    ...
    if (dnm < modLoads) {
      ...
      // load gradOutputs in bound  (BOTTLENECK !!)
...
(
 ...
 int filterSizeY, int filterSizeX, int moduleStrideY, int moduleStrideX
  ...
  if (nocolorCheck) {
    if ((colorsPerThread == 1) && (imgsPerThread == 4)) {
      cudaFuncSetCacheConfig( conv2d_update_gradInput < B_Y, B_X, 4, 1, dModuleCache, true >, cudaFuncCachePreferShared);
      conv2d_update_gradInput < B_Y, B_X, 4, 1, dModuleCache, true > <<<blocks, threads>>>
    ...
    } else if ((colorsPerThread == 1) && (imgsPerThread == 8)) {
      cudaFuncSetCacheConfig( conv2d_update_gradInput < B_Y, B_X, 8, 1, dModuleCache, true >, cudaFuncCachePreferShared);
      conv2d_update_gradInput < B_Y, B_X, 8, 1, dModuleCache, true > <<<blocks, threads>>>
        (gradOutputs, filters, gradInputs,
	 numImages, numImgColors, imgSizeY, imgSizeX,
	 numFilters, filterSizeY, filterSizeX,
	 numModulesY, numModulesX, moduleStrideY, moduleStrideX);
    } else if ((colorsPerThread == 2) && (imgsPerThread == 4)) {
      cudaFuncSetCacheConfig( conv2d_update_gradInput < B_Y, B_X, 4, 2, dModuleCache, true >, cudaFuncCachePreferShared);
      conv2d_update_gradInput < B_Y, B_X, 4, 2, dModuleCache, true > <<<blocks, threads>>>
        (gradOutputs, filters, gradInputs,
	 numImages, numImgColors, imgSizeY, imgSizeX,
	 numFilters, filterSizeY, filterSizeX,
	 numModulesY, numModulesX, moduleStrideY, moduleStrideX);
    } else if ((colorsPerThread == 2) && (imgsPerThread == 8)) {
      cudaFuncSetCacheConfig( conv2d_update_gradInput < B_Y, B_X, 8, 2, dModuleCache, true >, cudaFuncCachePreferShared);
      conv2d_update_gradInput < B_Y, B_X, 8, 2, dModuleCache, true > <<<blocks, threads>>>
        (gradOutputs, filters, gradInputs,
	 numImages, numImgColors, imgSizeY, imgSizeX,
	 numFilters, filterSizeY, filterSizeX,
	 numModulesY, numModulesX, moduleStrideY, moduleStrideX);
    } else if ((colorsPerThread == 4) && (imgsPerThread == 4)) {
      cudaFuncSetCacheConfig( conv2d_update_gradInput < B_Y, B_X, 4, 4, dModuleCache, true >, cudaFuncCachePreferShared);
      conv2d_update_gradInput < B_Y, B_X, 4, 4, dModuleCache, true > <<<blocks, threads>>>
        (gradOutputs, filters, gradInputs,
	 numImages, numImgColors, imgSizeY, imgSizeX,
	 numFilters, filterSizeY, filterSizeX,
	 numModulesY, numModulesX, moduleStrideY, moduleStrideX);
    } else if ((colorsPerThread == 4) && (imgsPerThread == 8)) {
      cudaFuncSetCacheConfig( conv2d_update_gradInput < B_Y, B_X, 8, 4, dModuleCache, true >, cudaFuncCachePreferShared);
      conv2d_update_gradInput < B_Y, B_X, 8, 4, dModuleCache, true > <<<blocks, threads>>>
        (gradOutputs, filters, gradInputs,
	 numImages, numImgColors, imgSizeY, imgSizeX,
	 numFilters, filterSizeY, filterSizeX,
	 numModulesY, numModulesX, moduleStrideY, moduleStrideX);
    ...
    if ((colorsPerThread == 1) && (imgsPerThread == 4)) {
      cudaFuncSetCacheConfig( conv2d_update_gradInput < B_Y, B_X, 4, 1, dModuleCache, false >, cudaFuncCachePreferShared);
      conv2d_update_gradInput < B_Y, B_X, 4, 1, dModuleCache, false > <<<blocks, threads>>>
        (gradOutputs, filters, gradInputs,
	 numImages, numImgColors, imgSizeY, imgSizeX,
	 numFilters, filterSizeY, filterSizeX,
	 numModulesY, numModulesX, moduleStrideY, moduleStrideX);
    } else if ((colorsPerThread == 1) && (imgsPerThread == 8)) {
      cudaFuncSetCacheConfig( conv2d_update_gradInput < B_Y, B_X, 8, 1, dModuleCache, false >, cudaFuncCachePreferShared);
      conv2d_update_gradInput < B_Y, B_X, 8, 1, dModuleCache, false > <<<blocks, threads>>>
        (gradOutputs, filters, gradInputs,
	 numImages, numImgColors, imgSizeY, imgSizeX,
	 numFilters, filterSizeY, filterSizeX,
	 numModulesY, numModulesX, moduleStrideY, moduleStrideX);
    } else if ((colorsPerThread == 2) && (imgsPerThread == 4)) {
      cudaFuncSetCacheConfig( conv2d_update_gradInput < B_Y, B_X, 4, 2, dModuleCache, false >, cudaFuncCachePreferShared);
      conv2d_update_gradInput < B_Y, B_X, 4, 2, dModuleCache, false > <<<blocks, threads>>>
        (gradOutputs, filters, gradInputs,
	 numImages, numImgColors, imgSizeY, imgSizeX,
	 numFilters, filterSizeY, filterSizeX,
	 numModulesY, numModulesX, moduleStrideY, moduleStrideX);
    } else if ((colorsPerThread == 2) && (imgsPerThread == 8)) {
      cudaFuncSetCacheConfig( conv2d_update_gradInput < B_Y, B_X, 8, 2, dModuleCache, false >, cudaFuncCachePreferShared);
      conv2d_update_gradInput < B_Y, B_X, 8, 2, dModuleCache, false > <<<blocks, threads>>>
        (gradOutputs, filters, gradInputs,
	 numImages, numImgColors, imgSizeY, imgSizeX,
	 numFilters, filterSizeY, filterSizeX,
	 numModulesY, numModulesX, moduleStrideY, moduleStrideX);
    } else if ((colorsPerThread == 4) && (imgsPerThread == 4)) {
      cudaFuncSetCacheConfig( conv2d_update_gradInput < B_Y, B_X, 4, 4, dModuleCache, false >, cudaFuncCachePreferShared);
      conv2d_update_gradInput < B_Y, B_X, 4, 4, dModuleCache, false > <<<blocks, threads>>>
        (gradOutputs, filters, gradInputs,
	 numImages, numImgColors, imgSizeY, imgSizeX,
	 numFilters, filterSizeY, filterSizeX,
	 numModulesY, numModulesX, moduleStrideY, moduleStrideX);
    } else if ((colorsPerThread == 4) && (imgsPerThread == 8)) {
      cudaFuncSetCacheConfig( conv2d_update_gradInput < B_Y, B_X, 8, 4, dModuleCache, false >, cudaFuncCachePreferShared);
      conv2d_update_gradInput < B_Y, B_X, 8, 4, dModuleCache, false > <<<blocks, threads>>>
        (gradOutputs, filters, gradInputs,
	 numImages, numImgColors, imgSizeY, imgSizeX,
	 numFilters, filterSizeY, filterSizeX,
	 numModulesY, numModulesX, moduleStrideY, moduleStrideX);
